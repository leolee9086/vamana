/**
 * Vamana Graph Index Implementation - MidiHeap Optimized Version
 * 基于Microsoft Research的DiskANN论文实现 + MidiHeap性能优化
 * 
...
// 创建MidiHeap的便利函数
function createMidiHeap<T>(capacity: number, compare: (a: T, b: T) => number) {
  return new MidiHeapGeneric<T>(capacity, compare);
}
...
export function createVamanaIndex(config: VamanaConfig = {}): VamanaIndex {
  ...
  function greedySearch(queryVector: Float32Array, startNodeId: number, beamSize: number): { candidates: SearchCandidate[], visited: Set<number> } {
    ...
    while (true) {
      ...
      // 🚀 保持候选集大小限制 - 使用MidiHeap优化Top-K选择
      if (candidates.length > beamSize) {
        // 创建最大堆，容量为beamSize，堆顶是距离最大的元素
        const heap = createMidiHeap<SearchCandidate>(
          beamSize,
          (a, b) => b.distance - a.distance  // 最大堆：距离大的在堆顶
        );
        
        // 将候选者添加到堆中，维护Top-K最小距离的元素
        for (const candidate of candidates) {
          if (!heap.isFull()) {
            heap.push(candidate);
          } else if (candidate.distance < heap.peek()!.distance) {
            heap.pop();           // 弹出距离最大的
            heap.push(candidate); // 插入新的更近的候选者
          }
        }
        
        // 从堆中获取结果并更新candidates数组
        const topKCandidates = heap.toSortedArray().reverse();
        candidates.length = 0; // 清空数组
        candidates.push(...topKCandidates); // 添加堆中的元素
  ...
  function robustPrune(nodeId: number, candidateIds: number[], alpha: number): number[] {
    ...
    // 🚀 使用MidiHeap对候选节点按距离排序
    const candidatesHeap = createMidiHeap<SearchCandidate>(
      allCandidates.size,
      (a: SearchCandidate, b: SearchCandidate) => a.distance - b.distance  // 最小堆：距离小的优先
    );
    ...
    for (const p of sortedCandidates) {
      ...
      for (const other of sortedCandidates) {
        if (p.id === other.id || newNeighbors.includes(other.id) || toRemove.has(other.id)) continue;
        
        const distPToOther = computeDistance(nodes[p.id].vector, nodes[other.id].vector);
  ...
  function insertNode(vector: Vector, data: NodeData = {}): number {
    ...
    if (useRobustPrune) {
      ...
      // 🚀 简单选择最近的R个邻居 - 使用MidiHeap优化
      const neighborsHeap = createMidiHeap<SearchCandidate>(
        R,
      ...
      newNode.neighbors = neighborsHeap.toSortedArray().map((c: SearchCandidate) => c.id);
    }
        ...
        if (!neighbor.neighbors.includes(nodeId)) {
          ...
          if (neighbor.neighbors.length > R) {
            if (useRobustPrune) {
              ...
              // 🚀 简单截断 - 保留距离最近的R个邻居
              const neighborCandidates: SearchCandidate[] = neighbor.neighbors.map((nId: number) => ({
                id: nId,
                distance: computeDistance(neighbor.vector, nodes[nId].vector)
              }));
              
              const topRHeap = createMidiHeap<SearchCandidate>(
                R,
                (a: SearchCandidate, b: SearchCandidate) => a.distance - b.distance
              );
              ...
              neighbor.neighbors = topRHeap.toSortedArray().map((c: SearchCandidate) => c.id);
            }
          }
        }
      }
  ...
  function searchKNN(queryVector: Vector, k = 10, searchParams: SearchParams = {}): SearchResult[] {
    ...
    // 🚀 使用MidiHeap选择最近的k个结果
    const topKHeap = createMidiHeap<SearchCandidate>(
      k,
      (a: SearchCandidate, b: SearchCandidate) => b.distance - a.distance // 最大堆: 保持距离最大的在顶部
    );
    
    // 正确的Top-K逻辑
    for (const candidate of searchResult.candidates) {
      if (topKHeap.length < k) {
      topKHeap.push(candidate);
      } else if (candidate.distance < topKHeap.peek()!.distance) { // 新的距离更小，则替换
        topKHeap.replace(candidate);
      }
    }
    
    // toSortedArray()返回从大到小排序的数组，反转后得到从小到大
    return topKHeap.toSortedArray().reverse().map((candidate: SearchCandidate) => ({
      id: candidate.id,
